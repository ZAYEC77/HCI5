...
namespace FileBrowser
{
    ...
    public partial class Browser : UserControl
    {
        ...
        private void copy()
        {
            String sourceDir = this.navAddressBox.SelectedItem.ToString();
            if (sourceDir == "Desktop") sourceDir = @"C:\Users\admin\Desktop";
            string destinationDir = "";
            FolderBrowserDialog f = new FolderBrowserDialog();
            if (f.ShowDialog() == DialogResult.OK)
            {
                destinationDir = f.SelectedPath;
            }
            ...
            foreach (ListViewItem item in fileView.SelectedItems)
            {
                ...
                try
                {
                    FileAttributes attr = File.GetAttributes(sourceDir + "\\" + i);
            ...
            foreach (string item in files)
            {
                try
                {
                    File.Copy(sourceDir + "\\" + item, destinationDir + "\\" + item);
                }
                catch (Exception e)
                {
            ...
            foreach (string item in directories)
            {
                try
                {
                    DirectoryCopy(sourceDir + "\\" + item, destinationDir + "\\" + item, true);
                }
                catch (Exception e)
                {
                    MessageBox.Show(e.Message);
        ...
        private void move()
        {
            ...
            foreach (string item in files)
            {
                File.Move(sourceDir + "\\" + item, destinationDir + "\\" + item);
            }
            foreach (string item in directories)
            {
                Directory.Move(sourceDir + "\\" + item, destinationDir + "\\" + item);
            }
            InitBrowser();
            friend.InitBrowser();
        }
